...
+(UINavigationController *)navigationControllerWithURL:(NSURL *)url {
    LWWKWebViewController *wkWebVC = [LWWKWebViewController new];
    wkWebVC.url = url;
    UINavigationController *navVC = [[UINavigationController alloc] initWithRootViewController:wkWebVC];
    wkWebVC.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Close", nil) style:UIBarButtonItemStylePlain target:wkWebVC action:@selector(dismissSelf)];
    return navVC;
}
...
- (void)performActivity {
    ...
    CGRect printableRect = CGRectMake(leftPadding,topPadding,kPaperSizeA6.width - leftPadding - rightPadding,kPaperSizeA6.height - topPadding - bottomPadding);
    CGRect paperRect = CGRectMake(0, 0, kPaperSizeA6.width, kPaperSizeA6.height);
