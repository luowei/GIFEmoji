...
+(BOOL)isQQInstalledWithView:(UIView *)view{
//    return [self canOpen:@"mqqapi://"];
    return [view canOpenURLWithString:@"mqqapi://"];
}
+(void)shareToQQFriends:(OSMessage*)msg fromView:(UIView *)view  Success:(shareSuccess)success Fail:(shareFail)fail{
    if ([self beginShare:schema Message:msg Success:success Fail:fail]) {
        [view openURLWithString:[self genShareUrl:msg to:0]];
    }
}
+(void)shareToQQZone:(OSMessage*)msg fromView:(UIView *)view  Success:(shareSuccess)success Fail:(shareFail)fail{
    if ([self beginShare:schema Message:msg Success:success Fail:fail]) {
        [view openURLWithString:[self genShareUrl:msg to:kQQAPICtrlFlagQZoneShareOnStart]];
    }
}
+(void)shareToQQFavorites:(OSMessage*)msg fromView:(UIView *)view  Success:(shareSuccess)success Fail:(shareFail)fail{
    if ([self beginShare:schema Message:msg Success:success Fail:fail]) {
        [view openURLWithString:[self genShareUrl:msg to:kQQAPICtrlFlagQQShareFavorites]];
    }
}
+(void)shareToQQDataline:(OSMessage*)msg fromView:(UIView *)view Success:(shareSuccess)success Fail:(shareFail)fail{
    if ([self beginShare:schema Message:msg Success:success Fail:fail]) {
        [view openURLWithString:[self genShareUrl:msg to:kQQAPICtrlFlagQQShareDataline]];
    }
}

+(void)QQAuth:(NSString*)scope fromView:(UIView *)view Success:(authSuccess)success Fail:(authFail)fail{
    if ([self beginAuth:schema Success:success Fail:fail]) {
        ...
        [view openURLWithString:[NSString stringWithFormat:@"mqqOpensdkSSoLogin://SSoLogin/tencent%@/com.tencent.tencent%@?generalpastboard=1",[self keyFor:schema][@"appid"],[self keyFor:schema][@"appid"]]];
    }
}
...
+(void)chatWithQQNumber:(NSString*)qqNumber fromView:(UIView *)view{
    [view openURLWithString:[NSString stringWithFormat:@"mqqwpa://im/chat?uin=%@&thirdAppDisplayName=%@&callback_name=%@&src_type=app&version=1&chat_type=wpa&callback_type=scheme",qqNumber,[self base64Encode:[self CFBundleDisplayName]],[self keyFor:schema][@"callback_name"]]];
}
+(void)chatInQQGroup:(NSString*)groupNumber fromView:(UIView *)view{
    [view openURLWithString:[NSString stringWithFormat:@"mqqwpa://im/chat?uin=%@&thirdAppDisplayName=%@&callback_name=%@&src_type=app&version=1&chat_type=group&callback_type=scheme",groupNumber,[self base64Encode:[self CFBundleDisplayName]],[self keyFor:schema][@"callback_name"]]];
}
@end
