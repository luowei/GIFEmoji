...
+(BOOL)isWeixinInstalledWithView:(UIView *)view{
    return [view canOpenURLWithString:@"weixin://"];
}
...
+(NSString*)genWeixinShareUrl:(OSMessage*)msg to:(int)shareTo{
    ...
    if (!msg.multimediaType) {
        ...
        if ([msg isEmpty:@[@"image",@"link"] AndNotEmpty:@[@"title"]]) {
            ...
            dic[@"thumbData"]=[OpenShare compressThumbnailData:(msg.thumbnail?:msg.image)];
            dic[@"objectType"]=@"2";
            if(msg.messageType == Msg_ImageGif){
                dic[@"objectType"]=@"8";
            }
...
//压缩缩略图大小
+ (NSData *)compressThumbnailData:(NSData *)thumbnailData {
//    //传言说thumbnail大小超过32k就不会动了,测试下来好像不是，与原图也有关系
//    UIImage *image = [UIImage imageWithData:thumbnailData];
//    while (image.memorySize > 32 * 1024) {  //大于32k则裁剪
//                CGSize tmpSize = CGSizeMake((CGFloat) (image.size.width * 0.8), (CGFloat) (image.size.height * 0.8));
//                image = [image scaleToSizeKeepAspect:tmpSize];
//            }
...
+(void)WeixinAuth:(NSString*)scope fromView:(UIView *)view Success:(authSuccess)success Fail:(authFail)fail{
    if ([self beginAuth:schema Success:success Fail:fail]) {
        [view openURLWithString:[NSString stringWithFormat:@"weixin://app/%@/auth/?scope=%@&state=Weixinauth",[self keyFor:schema][@"appid"],scope]];
    }
}
/**
